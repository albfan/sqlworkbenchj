...
public class Like
{
		...
		if (a.length > 0)
		{
			...
			if (escape != null)
			{
				...
				for (int i = 1; i < a.length; ++i)
				{
					...
					{
						last.image = last.image.substring(0, last.image.length() - esclen).concat(a[i].image);
	...
	private boolean next(String s, int length, int cursor, int element)
	{
		if (a.length > element)
		{
			...
			else
			{
				...
				if ((j <= length) && a[element].image.equals(s.substring(cursor, j)))
				{
	...
	{
		return "pattern: " + pattern + ", escape: " + escape + ", optimized pattern: " + tokensToString(a);
	}
