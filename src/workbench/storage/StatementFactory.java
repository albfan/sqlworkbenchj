...
public class StatementFactory
{
	...
	public DmlStatement createInsertStatement(RowData aRow, boolean ignoreStatus, String lineEnd, List<ColumnIdentifier> columns)
	{
		...
		boolean skipIdentityCols = Settings.getInstance().getGenerateInsertIgnoreIdentity();
		boolean useDefaultForIdentity = Settings.getInstance().getUseInsertDefaultForAutoincrement();
    ...
    sql.append("INSERT INTO ");
		...
		for (int col=0; col < cols; col ++)
		{
			...
			if (skipIdentityCols && colId.isAutoGenerated())
			{
				LogMgr.logDebug("StatementFactory.createInsertStatement()", "Ignoring column " + getTableNameToUse() + "." + colId.getColumnName() + " because it is an auto-increment column");
				continue;
			}
			...
			if (!colId.isAutoGenerated())
			{
				...
				{
					LogMgr.logDebug("StatementFactory.createInsertStatement()", "Ignoring column " + getTableNameToUse() + "." + colId.getColumnName() + " because it is a computed column");
					continue;
				}

				if (!colId.isUpdateable() || colId.isReadonly())
				{
					LogMgr.logDebug("StatementFactory.createInsertStatement()", "Ignoring column " + getTableNameToUse() + "." + colId.getColumnName() + " because it is marked as not updateable");
					continue;
				}
			}
			...
			if (includeCol)
			{
				...
				if (isNull && colId.isAutoGenerated() && useDefaultForIdentity)
				{
					valuePart.append("DEFAULT");
					// don't add a value to the values list, because we don't have a placeholder!
				}
				else
				{
