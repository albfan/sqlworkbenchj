	...
	extends SqlCommand
{
		...
		throws SQLException, Exception
	{
		...
		TableDataDiff diff = new TableDataDiff(sourceCon, targetCon);
		diff.setSqlDateLiteralType(literalType);
		
		TableDeleteSync deleteSync = new TableDeleteSync(targetCon, sourceCon);
		
		deleteSync.setRowMonitor(rowMonitor);
		diff.setRowMonitor(rowMonitor);
		...
		try
		{
			for (int i=0; i < tableCount; i++)
			{
				...
				try
				{
					diff.setOutputWriters(updates, inserts, nl);
					diff.setTableName(refTable, targetTable);
					diff.doSync();
				}
				finally
				{
		...
		if (result.isSuccess())
		{
			...
			try
			{
				...
				for (int i=0; i < len + 3; i++) line.append('-');
				line.append(nl);
				...
				for (TableIdentifier table : mapping.targetTables)
				{
					...
					if (ins.exists())
					{
						if (ins.length() > 0)
						{
							out.write("WbInclude -file='" + ins.getName() + "';" + nl);
							count ++;
						}
						else
						{
					...
					if (upd.exists())
					{
						if (upd.length() > 0)
						{
							out.write("WbInclude -file='" + upd.getName() + "';" + nl);
							count ++;
						}
						else
						{
				...
				for (TableIdentifier table : mapping.targetTables)
				{
					...
					if (f.exists())
					{
						if (f.length() > 0)
						{
							...
							out.write("WbInclude -file='" + f.getName() + "';" + nl);
							count ++;
						}
						else
						{
