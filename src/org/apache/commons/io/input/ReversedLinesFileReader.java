...
public class ReversedLinesFileReader implements Closeable {
    ...
    public ReversedLinesFileReader(final File file, final int blockSize, final Charset encoding) throws IOException {
        ...
        CharsetEncoder charsetEncoder = encoding.newEncoder();
        float maxBytesPerChar = charsetEncoder.maxBytesPerChar();
        if(maxBytesPerChar==1f) {
            // all one byte encodings are no problem
            byteDecrement = 1;
        } else if(encoding == Charset.forName("UTF-8")) {
            // UTF-8 works fine out of the box, for multibyte sequences a second UTF-8 byte can never be a newline byte
            // http://en.wikipedia.org/wiki/UTF-8
            byteDecrement = 1;
        } else if(encoding == Charset.forName("Shift_JIS")) {
            // Same as for UTF-8
            // http://www.herongyang.com/Unicode/JIS-Shift-JIS-Encoding.html
            byteDecrement = 1;
        } else if(encoding == Charset.forName("UTF-16BE") || encoding == Charset.forName("UTF-16LE")) {
            // UTF-16 new line sequences are not allowed as second tuple of four byte sequences,
            // however byte order has to be specified
            byteDecrement = 2;
        } else if(encoding == Charset.forName("UTF-16")) {
