...
(
  prd_details_id integer not null primary key,
...
(
   ...
   region_id          integer not null,
   prd_details_id     integer not null references product_details
);
...
alter table languages add constraint fk_lan_reg foreign key (language_id) references languages(language_id);
alter table product_details add constraint fk_pd_reg foreign key (region_id) references regions(region_id);
alter table product_details add constraint fk_pd_acc_mgr foreign key (acc_mgr_id) references account_mgr(acc_mgr_id);
