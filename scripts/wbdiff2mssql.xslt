...
ALTER TABLE <xsl:value-of select="$table"/> ALTER COLUMN <xsl:value-of select="$column"/><xsl:text> </xsl:text><xsl:value-of select="dbms-data-type"/>;
   </xsl:if>
   <xsl:if test="nullable = 'true'">
ALTER TABLE <xsl:value-of select="$table"/> ALTER COLUMN <xsl:value-of select="$column"/><xsl:text> </xsl:text><xsl:value-of select="dbms-data-type"/> NULL;
   </xsl:if>
   <!-- SQL Server cannot change a column to NOT NULL 
   <xsl:if test="nullable = 'false'">
ALTER TABLE <xsl:value-of select="$table"/> ALTER COLUMN <xsl:value-of select="$column"/> NOT NULL;
   </xsl:if>
   -->
   <!-- SQL Server 2000 cannot change the default value without dropping the column ... 
   <xsl:if test="string-length(default-value) &gt; 0">
ALTER TABLE <xsl:value-of select="$table"/> ALTER COLUMN <xsl:value-of select="$column"/> DEFAULT <xsl:value-of select="default-value"/>;  
   </xsl:if>
   -->
</xsl:template>
...
DROP VIEW <xsl:value-of select="view-name"/>;
...
(
  <xsl:for-each select="column-def">
    ...
    <xsl:variable name="uppername">
      <xsl:value-of select="translate(column-name,'abcdefghijklmnopqrstuvwxyz','ABCDEFGHIJKLMNOPQRSTUVWXYZ')"/>  
    </xsl:variable>
